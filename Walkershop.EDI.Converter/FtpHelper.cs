...
namespace Walkershop.EDI.Converter
{
    ...
    {
        static ILog logger = log4net.LogManager.GetLogger(typeof(FtpHelper));
        ...
        /// <param name="remoteFilePath">ftp目的路径</param>
        /// <param name="ftpUserName">user</param>
        /// <param name="ftpPwd">password</param>
        public static bool Upload(string localFilePath, string remoteFilePath, string userName, string password)
        {
            ...
            {                
                ...
                FileStream myStream = new FileStream(localFilePath, FileMode.Open, FileAccess.Read);
                byte[] dataByte = new byte[myStream.Length];
                myStream.Read(dataByte, 0, dataByte.Length);  //写到2进制数组中
                myStream.Close();
                if (dataByte.Length > 0)
                {
                    request.UploadData(remoteFilePath, dataByte);
                }
                else
                {
                    logger.Info("0 byte size file [" + localFilePath + "].");
                }

                logger.Info("Upload file [" + localFilePath + "] to FTP done.");
                return true;
            }
            catch (Exception e)
            {
                logger.Error("Upload file [" + localFilePath + "] to FTP Error:" + e.Message);
                return false;
            }
            finally 
            {
        ...
        public static bool Upload (string localFilePath, string remoteFilePath, string userName, string password, int retryTimes)    
        {
            // FTP 重试功能
            int retry_times = 0;
            bool success = false;
            while (!success && retry_times < retryTimes)
            {
                try
                {
                    retry_times++;

                    logger.Info(string.Format("Trying to upload file to FTP {0} times .....", retry_times));

                    success = FtpHelper.Upload(localFilePath, remoteFilePath, userName, password);                    
                }
                catch (Exception ex)
                {
                    success = false;
